...
`opts`: `{ testCmd = ['npm','test'], cwd = process.cwd(), cap = 80 }`.
...
4. For each mutant: write it to `srcPath`, run `testCmd` (child env has `NODE_TEST_CONTEXT` scrubbed).
   - child exit `0` ⇒ tests passed ⇒ **mutant SURVIVED** (that line is test-theatre).
   - child exit non-zero ⇒ tests failed ⇒ **mutant KILLED** (the line is guarded).
5. `finally`: write `original` back to `srcPath`; delete the sidecar.

`Result`: `{ total, capped, killed, survived[], score = killed/total, clean = survived.length === 0 }`.
Each `survived` entry: `{ line, mutation: 'from → to', snippet }`.
